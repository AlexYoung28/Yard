...
export default function FeedingSchedule({ staffList, data, setData }) {
  ...
  const [dayMobile, setDayMobile] = useState(days[(new Date().getDay() + 6) % 7]);
  ...
  if (isMobile) {
    return (
      <section className="space-y-4 p-4">
        <h2 className="text-xl font-bold text-amber-700 dark:text-amber-300">Feeding Schedule</h2>
        {/* Day tabs */}
        ...
        {summary && (
          <div className="px-3 py-2 bg-blue-100 dark:bg-blue-900 rounded text-sm">
            First: <strong>{summary.first.time}</strong> by <strong>{summary.first.staff}</strong> •
            Last:  <strong>{summary.last.time}</strong> by <strong>{summary.last.staff}</strong>
          </div>
        )}
                  ...
                  <div className="bg-white dark:bg-gray-700 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={e => change(staffName, t.key, dayMobile, e.target.value)}
                      />
                    </div>
                  </div>
                )}
  ...
  return (
     <SectionCard title="Feeding Schedule" accent="amber">
      ...
      <div className="relative">
        <div ref={scrollHook.ref} className="overflow-auto max-h-[60vh] rounded-xl">
          <table className="w-full border-collapse text-sm min-w-max">
            <thead>
              <tr className="bg-amber-100 dark:bg-amber-900 text-slate-800 dark:text-amber-100">
                <th className="border p-2 sticky top-0 bg-inherit text-left font-semibold">Staff / Time</th>
                <th className="border p-2 sticky top-0 bg-inherit font-semibold">{dayMobile}</th>
              </tr>
            </thead>
            <tbody>
              {staffList.map(person =>
                feedingTypes.map(t => (
                  <tr key={`${person}-${t.key}`} className="even:bg-amber-50/40 dark:even:bg-amber-900/30 hover:bg-slate-100 dark:hover:bg-slate-700">
                    <td className="border p-1 font-medium whitespace-nowrap">{person} {t.label}</td>
                    <td className="border p-1">
                      <TextCell
                        ...
                        onFocus={() => (active.current = { staff: person, typeKey: t.key, day: dayMobile })}
                      />
                    </td>
                  </tr>
                ))
