 ...
 */
export default function DutiesSchedule({ horses, data, setData, weekLabel, dutyOptions }) {
  const [selectedDay, setSelectedDay] = useState(days[(new Date().getDay() + 6) % 7]);
  const scrollHook = useScrollShadows();

  const uniqueOptions = useMemo(() => Array.from(new Set(dutyOptions)), [dutyOptions]);
  ...
  const rows = useMemo(
    () => horses.flatMap(horse =>
      dutyTasks.map(task => (
        ...
        >
          <td
            className="border p-1 sticky left-0 z-10 bg-white dark:bg-slate-800 font-medium whitespace-nowrap text-slate-900 dark:text-slate-100"
          >
            {horse} {task}
          </td>
          <td className="border p-1 min-w-[160px]">
            <select
              ...
              onChange={e => change(horse, task, selectedDay, e.target.value)}
            >
              <option value="">—</option>
  ...
  return (
    <SectionCard
      title="Duties Rota"
      accent="emerald"
      right={<span className="text-sm text-slate-700 dark:text-slate-300">{weekLabel}</span>}
    >
      ...
      <div className="relative">
        <div ref={scrollHook.ref} className="overflow-auto max-h-[60vh] rounded-xl [-webkit-overflow-scrolling:touch]">
          <table className="w-full border-collapse text-sm min-w-max">
            <thead>
              <tr className="bg-emerald-100 dark:bg-emerald-900 text-slate-800 dark:text-emerald-100">
                <th className="border p-2 sticky top-0 bg-inherit text-left font-semibold">Task</th>
                <th className="border p-2 sticky top-0 bg-inherit font-semibold">{selectedDay}</th>
              </tr>
            </thead>
            <tbody>{rows}</tbody>
          </table>
