...
export default function EditableList({ title, items, setItems, placeholder }) {
  ...
  const update = (i, v) =>
    setDraft((prev) => prev.map((s, idx) => (idx === i ? v : s)));
        ...
        <>
          {draft.map((n, i) => (
            <div key={i} className="flex items-center gap-2 mb-1">
              ...
              />
              <button onClick={() => remove(i)} className="px-2 text-red-600">
                Delete
              </button>
            </div>
          ))}
          <div className="flex gap-2 mt-2">
            <button onClick={add} className="px-2 py-1 border rounded">
              + Add
            </button>
            <button
              onClick={save}
            ...
            <button onClick={cancel} className="px-2 py-1 border rounded">
              Cancel
            </button>
          </div>
        </>
