...
export default function useMediaQuery(q){
  ...
  useEffect(() => {
    ...
    const m = window.matchMedia(q); const fn = () => setOk(m.matches);
    if (m.addEventListener) m.addEventListener("change", fn); else if (m.addListener) m.addListener(fn);
    fn();
    return () => { if (m.removeEventListener) m.removeEventListener("change", fn); else if (m.removeListener) m.removeListener(fn); };
  },[q]);
  return ok;
}